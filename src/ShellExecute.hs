...
-}
runCommandWithIO 
    -- | 'Just' the shell to run as path or in case of 'Nothing' to call directly without additional shell
    :: Maybe FilePath 
    -- | 'Just' the working directory to set or in case of 'Nothing' take the working directory as is
    -> Maybe FilePath 
    -- | the command to execute
    -> String -> 
    -- | the arguments to hand over to the command to execute
    [String] 
    -- | a string to be passed as stdin
    -> Maybe String 
    -- | Just a tupple of output (stdout, stderr, and exit code) generated by the command
...
runInteractiveCommandInDir msShell msWorkingDirectory sCommand lsArgs = do
        ...
        return
            (Sfr.ifJust3
                ...
                Nothing)
    where
        processSpec = Sfr.ifJust msShell (\sShell -> Proc.proc sShell (["-c", sCommand] ++ lsArgs)) (Proc.proc sCommand lsArgs)
